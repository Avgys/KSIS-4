...
namespace SmtpProtocol
{
    internal class Program
    {
        ...
        {
            Console.Write("SMTP\n");
            var server = new Server(25, new AuthorizedClient("AvgysFistrick@yandex.ru", "igonas97epifot45"));
            const string command = "/changer";
            const string exit = "/exit";

            Console.Write("Enter Adress: ");
            Client receiver = new Client(Console.ReadLine());
            Console.WriteLine(String.Join(" ","Enter command to change reciver",command));
